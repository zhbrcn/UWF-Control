...
function Write-Log {
    ...
    Add-Content -Path $Script:LogPath -Value "[$time] $Message" -Encoding UTF8
}
...
function Relaunch-AsAdmin {
    Write-Host '检测到当前不是管理员权限，正在尝试提权重启脚本...' -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host '无法自动提权，请右键选择 以管理员身份运行。' -ForegroundColor Red
...
function Run-Command {
    ...
    try {
        ...
        if (-not $IgnoreError) { return $null }
        return ""
    }
}
...
function Confirm-Yes {
    ...
    $v = (Read-Host "$Prompt (输入 $Keyword 继续)").Trim()
    if ($v -ceq $Keyword) { return $true }
    Write-Host '已取消操作。' -ForegroundColor Yellow
    return $false
}

function Ask-Reboot {
    $ans = (Read-Host '该操作需要重启后生效，是否立即重启? (Y/N)').Trim()
    if ($ans -match '^[Yy]$') { Run-Command 'shutdown /r /t 0' | Out-Null }
}

function Pause-Return { Read-Host "`n按 Enter 返回主菜单" | Out-Null }
...
function Show-SimpleStatus {
    $cfg = Run-Command 'uwfmgr get-config' -IgnoreError
    $ov = Run-Command 'uwfmgr overlay get-config' -IgnoreError
    $vol = Run-Command 'uwfmgr volume get-config' -IgnoreError
    $fex = Run-Command 'uwfmgr file get-exclusions all' -IgnoreError
    $rex = Run-Command 'uwfmgr registry get-exclusions' -IgnoreError
    ...
    try {
        ...
        $ovType = ([regex]::Match($ov,'(?im)Type\s*:\s*(.+)$')).Groups[1].Value.Trim()
        $ovSize = ([regex]::Match($ov,'(?im)Maximum\s+Size\s*:\s*(.+)$')).Groups[1].Value.Trim()
        $warn = ([regex]::Match($ov,'(?im)Warning\s+Threshold\s*:\s*(.+)$')).Groups[1].Value.Trim()
        $crit = ([regex]::Match($ov,'(?im)Critical\s+Threshold\s*:\s*(.+)$')).Groups[1].Value.Trim()
        $fileCount = ([regex]::Matches($fex,'(?im)^\s*[A-Za-z]:\\')).Count
        $regCount = ([regex]::Matches($rex,'(?im)^\s*HKEY_')).Count
        ...
        $needReboot = '否'
        if (($currentFilter -ne '未知' -and $nextFilter -ne '未知' -and $currentFilter -ne $nextFilter) -or ($currentC -ne '未知' -and $nextC -ne '未知' -and $currentC -ne $nextC) -or $Script:PendingReboot) { $needReboot='是' }
...
function Show-FullStatus {
    Run-Command 'uwfmgr get-config' -IgnoreError | Out-Null
    Run-Command 'uwfmgr volume get-config' -IgnoreError | Out-Null
    Run-Command 'uwfmgr overlay get-config' -IgnoreError | Out-Null
    Run-Command 'uwfmgr file get-exclusions all' -IgnoreError | Out-Null
    Run-Command 'uwfmgr registry get-exclusions' -IgnoreError | Out-Null
}
...
function Enable-RecommendedUWF {
    ...
    Write-Host '- 不会保护 D:\'
    Write-Host '- 需要重启生效'
    if (-not (Confirm-Yes '确认继续吗?' 'YES')) { return }
    Run-Command 'powercfg /h off' -IgnoreError | Out-Null
    Run-Command 'uwfmgr overlay set-size 16384' -IgnoreError | Out-Null
    Run-Command 'uwfmgr volume protect C:\' -IgnoreError | Out-Null
    Run-Command 'uwfmgr filter enable' -IgnoreError | Out-Null
...
function Disable-UWF {
    ...
    Write-Host '- 重启后生效'
    if (-not (Confirm-Yes '确认继续吗?' 'YES')) { return }
    Run-Command 'uwfmgr filter disable' -IgnoreError | Out-Null
    $Script:PendingReboot = $true
    Ask-Reboot
}
...
function Set-OverlaySize {
    ...
    switch ($choice) {
        ...
        '3' { $size=32768 }
        '4' {
            $in = (Read-Host '请输入自定义 MB (正整数)').Trim()
            $tmp = 0
            if (-not [int]::TryParse($in,[ref]$tmp) -or $tmp -le 0) { Write-Host '输入无效，必须是正整数。' -ForegroundColor Yellow; return }
            $size = $tmp
        }
        '0' { return }
        default { Write-Host '无效选项。' -ForegroundColor Yellow; return }
    }
    if ($size -lt 1024) { Write-Host '警告: 小于 1024MB 通常不建议。' -ForegroundColor Yellow }
    if ($size -gt 65536) { if (-not (Confirm-Yes '该值较大，确认继续吗?' 'YES')) { return } }
    Run-Command "uwfmgr overlay set-size $size" -IgnoreError | Out-Null
    $Script:PendingReboot = $true
...
function Add-FileExclusion {
    ...
    if ([string]::IsNullOrWhiteSpace($path)) { Write-Host '路径不能为空。' -ForegroundColor Yellow; return }
    $normalized = $path.TrimEnd('\\')
    $dangerRoots = @('C:','C:\Windows','C:\Users','C:\Users\Brz','C:\ProgramData','C:\Program Files','C:\Program Files (x86)')
    foreach ($root in $dangerRoots) {
        $r = $root.TrimEnd('\\')
        if ($normalized -ieq $r) { Write-Host "拒绝: 不允许直接排除高风险大目录: $root" -ForegroundColor Red; return }
    }
    foreach ($root in $dangerRoots) {
        $r = $root.TrimEnd('\\')
        if ($normalized.Length -lt ($r.Length + 3) -and $normalized -like "$r*") { Write-Host "警告: 路径过于接近高风险大目录: $root，已拒绝。" -ForegroundColor Red; return }
    }
    if (-not (Test-Path -LiteralPath $path)) {
        $ans = (Read-Host '路径不存在，是否仍要添加? (Y/N)').Trim()
        if ($ans -notmatch '^[Yy]$') { Write-Host '已取消。' -ForegroundColor Yellow; return }
    }
    Run-Command "uwfmgr file add-exclusion `"$path`"" -IgnoreError | Out-Null
    $Script:PendingReboot = $true
...
function Remove-FileExclusion {
    ...
    if ([string]::IsNullOrWhiteSpace($path)) { Write-Host '路径不能为空。' -ForegroundColor Yellow; return }
    if (-not (Confirm-Yes '确认删除该排除项吗?' 'YES')) { return }
    Run-Command "uwfmgr file remove-exclusion `"$path`"" -IgnoreError | Out-Null
    $Script:PendingReboot = $true
...
function Create-UWFTestItems {
    "uwf test" | Out-File "$env:USERPROFILE\Desktop\uwf-test.txt" -Encoding UTF8
    New-Item -ItemType Directory -Path 'C:\uwf-test-folder' -Force | Out-Null
    cmd.exe /c 'reg add "HKCU\Software\UWFTest" /v TestValue /t REG_SZ /d "hello" /f' | Out-Null
    Write-Log '创建测试项: 文件/目录/注册表'
...
function Clear-UWFConfig {
    ...
    Write-Host '- 重启后生效'
    if (-not (Confirm-Yes '确认继续吗?' 'CLEAR')) { return }
    Run-Command 'uwfmgr filter disable' -IgnoreError | Out-Null
    Run-Command 'uwfmgr volume unprotect C:\' -IgnoreError | Out-Null
    $Script:PendingReboot = $true
    Ask-Reboot
}
...
while ($true) {
    ...
    switch ($choice) {
        ...
        '11' {
            if (Confirm-Yes '确认立即重启电脑吗?' 'REBOOT') { Run-Command 'shutdown /r /t 0' | Out-Null }
            Pause-Return
        }
        '12' { Clear-UWFConfig; Pause-Return }
        '0' { Write-Log '用户退出脚本'; break }
        default { Write-Host '无效选项，请重新输入。' -ForegroundColor Yellow; Pause-Return }
    }
}
